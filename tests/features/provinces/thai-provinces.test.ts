...
describe("thaiProvinces", () => {
  it("matches the authoritative 77-province code and name mapping", () => {
    expect(thaiProvinces).toEqual([
  ...
  it("provides each of Thailand's 77 provinces with a unique standard code", () => {
    expect(thaiProvinces).toHaveLength(77)
    expect(new Set(thaiProvinces.map((province) => province.code)).size).toBe(77)
  })

  it("finds Trang from its standard province code", () => {
    expect(findProvinceByCode("92")).toMatchObject({
      code: "92",
      nameTh: "ตรัง",
      nameEn: "Trang",
    })
  ...
  it("searches province names in English without changing the canonical code", () => {
    expect(searchThaiProvinces("chiang")).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ code: "50", nameTh: "เชียงใหม่" }),
        expect.objectContaining({ code: "57", nameTh: "เชียงราย" }),
      ]),
    )
  })

  it("searches province names in Thai", () => {
    expect(searchThaiProvinces("ตรัง")).toEqual([
      expect.objectContaining({ code: "92", nameEn: "Trang" }),
    ])
  })
})
